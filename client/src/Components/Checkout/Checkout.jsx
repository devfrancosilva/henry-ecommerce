...
export function ContactUs({ userLogged, clearCart }) {
              ...
              fetch('http://localhost:3002/orders/user/checkout', {
                ...
                .then((response) => {
                  if (response.status === 500) {
                    swal(
                      'No hay stock disponible. No podemos precesar su compra.'
                    )
                  } else {
                    emailjs.sendForm(serviceID, templateID, value, name)
                    swal(
                      'Tu compra fue procesada con exito. En breve recibiras un e-mail de confirmación'
                    ).then((res) => {
                      if (res) {
                        clearCart()
                        history.push('/')
                ...
                .catch(() =>
                  swal('Error al finalizar su compra. Intentar otra vez!')
                )
            }}
          >
